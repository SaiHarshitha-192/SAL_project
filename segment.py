...
if __name__ == "__main__":
    ...
    print("Starting audio segmentation process...", flush=True)
    ...
    for batch_i in tqdm.tqdm(range(0, len(wav_files), batch_size)):
        ...
        print(f"Processing batch {batch_i // batch_size + 1} with files: {[wf.name for wf in wav_file_batch]}", flush=True)
        outputs = mincut(segmenter(wav_file_batch))
        print(f"Processed {len(outputs)} files in this batch.", flush=True)
        if not outputs:
            print("No outputs generated for this batch.", flush=True)

        for wf, output in zip(wav_file_batch, outputs):
            np.save(output_dir / f"{wf.stem}_feature.npy", output["features"])
            print(f"Saved features to {output_dir / f'{wf.stem}_feature.npy'}", flush=True)
            np.save(output_dir / f"{wf.stem}_segmentfeature.npy", output["segment_features"])
            print(f"Saved segment features to {output_dir / f'{wf.stem}_segmentfeature.npy'}", flush=True)
            with open(output_dir / f"{wf.stem}_segment.txt", "w") as f:
                for s, e in output["segments"]:
                    line = f"{s:.03f}, {e:.03f}\n"
                    f.write(line)
            print(f"Saved segments to {output_dir / f'{wf.stem}_segment.txt'}", flush=True)
